...
export class UserRolesDrawer {
  ...
  set data(userDirty: UserDirty) {
    ...
    this.currentLogin = userDirty.login
  }
  ...
  form: FormGroup<Form> = new FormGroup<Form>({
    login: new FormControl<string>('', {
      ...
      asyncValidators: [this.canUseLogin()]
    }),
    isMyProfile: new FormControl<boolean>(false, { nonNullable: true }),
    roles: new FormControl<string[]>([], { nonNullable: true })
  })

  private canUseLogin(): AsyncValidatorFn {
    console.log(this.currentLogin)
    return (control: AbstractControl): Observable<ValidationErrors | null> => {
      return control.value === this.currentLogin
        ? of(null)
        : this.service.hasLogin(control.value).pipe(
          map((exists) =>
